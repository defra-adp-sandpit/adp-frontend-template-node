...
Create a new repository from this template and run `./rename.js` specifying the new name of the project, description and frontend type (internal/external)to use e.g.
```
 node rename.js test-web-frontend 'test-web-frontend external' external
 node rename.js test-web-frontend 'test-web-frontend internal' internal
```
...
* Rename `helm/adp-frontend-template-node` and `helm/adp-frontend-template-node-infra`
* Updates the Chart name, description and templates in `helm/adp-frontend-template-node/templates` and `helm/adp-frontend-template-node-infra/templates`
* Updates the project name in `.azuredevops/build.yaml`

### Notes on automated rename

* The Helm chart deployment values in `helm/*/values.yaml` may need updating depending on the resource needs of your microservice
